...
export default tool({
    ...
    async execute(args, _context) {
        args.workingDirectory && await Bun.$.cwd(args.workingDirectory).nothrow().quiet();
        ...
        if(prNumberString) {
            ...
            const gitPRCheckoutCommand:string= `git fetch origin pull/${prNumber}/head:${branchName}`.nothrow().quiet();
            await Bun.$(gitPRCheckoutCommand).nothrow().quiet();;
